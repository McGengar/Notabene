...
var text = [
	"[center]Try to break through[/center]",
	"[center]Strong enemy appeared[/center]"
...
func _ready():
	...
	if Globals.level_4_time==0:
		...
		fading_in = false
		await get_tree().create_timer(2).timeout
	elif Globals.level_4_time==1:
		time = 3
		$AudioStreamPlayer2D.stop()
		$Enemies/Spawner.free()
		$Enemies/Spawner1.free()
		boss_spawned=2
	#fading_in = true
...
func _process(delta):
	...
	if boss_spawned==1 and $Enemies.get_node("BigMetalEnemy") == null:
		boss_spawned=2
		$AudioStreamPlayer2D.stop()
		await get_tree().create_timer(5).timeout
		Globals.level_4_time=1
	
	if boss_spawned==3 and $Enemies.get_node("BigEarthEnemy") == null :
		boss_spawned=4
